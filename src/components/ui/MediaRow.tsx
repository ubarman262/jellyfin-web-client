...
const MediaRow: React.FC<MediaRowProps> = ({ title, items, isLoading = false }) => {
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="w-full aspect-[2/3] bg-gray-800 animate-pulse rounded-md"></div>
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="mb-8 group/row">
      ...
      <div className="relative">
        <div 
          ref={rowRef}
          className="flex overflow-x-scroll scrollbar-hide gap-2 pb-4"
